...
export default function CampagnePrenotato({ campagnaId, campagnaLabel, token, titoli, onImportDone }) {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const result = Object.values(aggMap).map(r => {
          const t = titoli.find(t => t.ean === r.ean || t.ean === String(parseInt(r.ean)));
          return { ...r, titolo: t?.titolo ?? "— non trovato —", found: !!t, titolo_id: t?.id };
  ...
  const totale = aggregato.reduce((s, r) => s + r.qta, 0);
  ...
  const reset = () => { setRighe([]); setAggregato([]); setDone(null); setStep("upload"); };
      ...
      {step === "preview" && (
        <div>
          <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
            {[["Righe lette", righe.length.toLocaleString("it"), T.text], ["Totale copie", totale.toLocaleString("it"), T.accent], ["Trovati in cedola", totaleFound.toLocaleString("it"), T.green], ["Non trovati", aggregato.filter(r => !r.found).length, T.red]].map(([label, val, color]) => (
